...
const authenticate = async () => {
  try {
    ...
    console.log(invalidEmail.value, invalidPassword.value)

    if (invalidEmail.value || invalidPassword.value) {
      toast.add({
        severity: 'info',
        summary: 'Login',
        detail: 'Preencha os campos abaixo corretamente',
        life: 3000,
      })
    ...
    const { status, response } = await Auth.authenticate(
      email.value,
      password.value,
    )

    if (status === 200) {
      useUser.setUser(response.body)
    ...
    if (status >= 500 || status <= 599) {
      toast.add({
        severity: 'error',
        detail: 'Ops! Ocorreu um erro tente novamente em breve!',
        life: 3000,
      })
      return
    }
