<template>
  ...
  <ModalCategories
    ...
    @close:modal="closeCategoryDialog"
  ...
  >
    ...
    <div class="mb-4 mt-2">
      <FloatLabel variant="on">
        <InputNumber
          ...
          inputId="valorTransacao"
          mode="currency"
          currency="BRL"
          locale="pt-BR"
          fluid
          :invalid="transaction.value < 0"
        />
    ...
    <div class="mb-4 mt-2">
      <FloatLabel variant="on">
        <Select
          ...
          filter
          filterPlaceholder="Digite a conta bancaria"
          optionLabel="name_bank"
          class="w-full"
          inputId="accounts"
          :invalid="submitted && !transaction.category.name"
          placeholder="Selecione a conta bancaria"
        >
          <template #header>
    ...
    <div class="mb-4 mt-2">
      <FloatLabel variant="on">
        <Select
          ...
          filter
          filterPlaceholder="Digite o meio de pagamento"
          optionLabel="name"
          class="w-full"
    ...
    <div class="mb-4 mt-2">
      <FloatLabel variant="on">
        <Select
          ...
          filter
          filterPlaceholder="Digite a categoria"
          optionLabel="name"
          class="w-full"
    ...
    <div class="mb-4 mt-2" v-if="transaction.format === 'Recorrente'">
      <FloatLabel variant="on">
        <Select
          v-model="transaction.amount_recurrence"
          :options="recurrence"
          class="w-full"
          inputId="recurrence"
          :invalid="submitted && !transaction.category.amount_recurrence <= 0"
        />
...
const isValidForm = () => {
  if (!transaction.value.name) return false
  if (!transaction.value.value < 0) return false
  if (!transaction.value.payment_method) return false
  if (!transaction.value.category.name) return false
  if (!transaction.value.format) return false
  if (!transaction.value.operation) return false
  if (
    transaction.value.amount_recurrence <= 0 &&
    transaction.value.format === 'Recorrente'
  )
    return false
  if (!transaction.value.account) return false
  return true
}
