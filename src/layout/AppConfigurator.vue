...
const updateColors = (type, color) => {
  ...
  localStorage.setItem('UpdateColors', JSON.stringify({ type, color }))
}
...
const applyTheme = (type, color) => {
  ...
  localStorage.setItem('ApplyTheme', JSON.stringify({ type, color }))
}
...
const init = () => {
  const storedColors = JSON.parse(localStorage.getItem('UpdateColors'))
  if (storedColors) {
    updateColors(storedColors.type, storedColors.color)
  }

  const getApplyTheme = JSON.parse(localStorage.getItem('ApplyTheme'))
  if (getApplyTheme) {
    applyTheme(getApplyTheme.type, getApplyTheme.color)
  }
}
